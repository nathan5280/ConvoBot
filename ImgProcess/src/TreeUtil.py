...
class TreeUtil(object):
    ...
    def __init__(self, src_root, trg_root):
        ...
        Input:
            ...
            trg_root: Target root directory.   This will always be created.
        '''
        self._src_root = src_root
        self._trg_root = trg_root

        # Create the target directory if it doesn't exist.
        if os.path.exists(trg_root):
            self._cur_trg_dir = trg_root
        else:
            os.makedirs(trg_root)
    ...
    def apply_files(self, funct, pattern='*', copy_dir=True):
        ...
        for os_entry in self._traverse_dir():
            ...
            trg_path = os.path.join(self._trg_root, rel_path)

            for filename in os_entry[2]:
                if fnmatch.fnmatch(filename, pattern):
                    funct(src_path, trg_path, filename)
    ...
    def dup_tree(self):
        ...
        def create_dir(dir):
            ...
            # then adding it back onto the trg_root.
            rel_path = dir[len(self._src_root)+1:]
            new_path = os.path.join(self._trg_root, rel_path)
    ...
    def copy_files(self, fn_pattern):
        ...
        '''
        def copy_file(src_path, trg_path, filename):
            '''
            Helper method to copy the files that match the pattern.
            '''
            copyfile(os.path.join(src_path, filename), os.path.join(trg_path, filename))
