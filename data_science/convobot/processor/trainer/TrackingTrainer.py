...
class TrackingTrainer(Trainer):
    ...
    def process(self) -> None:
        ...
        split_data_mgr = SplitDataMgr(self.src_dir_path)

        # Get the hyper and training parameters.
        self._sessions = self.parameters['Sessions']
        self._epochs = self.parameters['Epochs']
        self._batch_size = self.parameters['BatchSize']
        self._lr = self.parameters['LearningRate']

        logger.info(
            'Session: {}, Epochs: {}, Batch Size: {}, Learning Rate: {}'. format(
                self._sessions, self._epochs, self._batch_size, self._lr))

        # Set up the tensor board callback to generate output files to review
        # the training progress.
        tb_path = os.path.join(self.dst_dir_path, 'graph')
        ...
        for session in range(self._sessions):
            logger.info(
                'Session: {} of {}'.format(session + 1, self._sessions))

            # Train!
            model.fit(split_data_mgr.train_image, split_data_mgr.train_label,
            ...
            score = model.evaluate(split_data_mgr.test_image, split_data_mgr.test_label, verbose=0)
